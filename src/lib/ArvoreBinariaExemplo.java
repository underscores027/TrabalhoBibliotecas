...
public class ArvoreBinariaExemplo<T> implements IArvoreBinaria<T> {
    ...
    @Override
    public List<T> caminharEmNivel() {
        List<T> result = new ArrayList<>();
        if (raiz == null) {
            return result; // Retorna lista vazia caso a árvore seja vazia.
        }
        ...
        while (!fila.isEmpty()) {
            ...
            result.add(noAtual.getValor()); // Adiciona o valor do nó à lista.
    ...
    @Override
    public List<T> caminharEmOrdem() {
        List<T> resultado = new ArrayList<>();
        caminharEmOrdemRecursivo(raiz, resultado);
        return resultado;
    }

    private void caminharEmOrdemRecursivo(No<T> no, List<T> resultado) {
        if (no == null) {
            return;
        }
    ...
    public List<T> obterTodos() {
        List<T> resultado = new ArrayList<>();
        obterTodosRecursivo(raiz, resultado);
        return resultado;
    }

    private void obterTodosRecursivo(No<T> no, List<T> resultado) {
        if (no == null) {
            return;
        }

        obterTodosRecursivo(no.getFilhoEsquerda(), resultado);
        resultado.add(no.getValor());
        obterTodosRecursivo(no.getFilhoDireita(), resultado);
    }
    @Override
